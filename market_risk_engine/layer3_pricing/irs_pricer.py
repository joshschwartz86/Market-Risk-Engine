"""Interest Rate Swap pricers: vanilla IRS, amortizing IRS, float-float, and amortizing float-float."""
...
from ..layer2_portfolio.models import (
    AmortizingFloatFloatSwap, AmortizingIRS, FloatFloatSwap, IRS, PayReceive,
)
from .base import MarketSnapshot, PricingEngine, PricingResult


# ---------------------------------------------------------------------------
# Spread-adjusted discount factor helper
# ---------------------------------------------------------------------------

def _sdf(disc: YieldCurveInterpolator, t: float, spread: float) -> float:
    """
    Discount factor adjusted for an additive zero-rate spread ``s``.

        DF_adj(t) = DF(t) * (1 + s)^(-t)

    The spread is expressed in decimal (e.g. 0.0010 for 1 bp).
    """
    if t <= 0.0:
        return 1.0
...
class IRSPricer(PricingEngine):
    ...
    def _fixed_leg_pv(self, trade: IRS, disc: YieldCurveInterpolator,
        ...
        freq = trade.fixed_payment_frequency or trade.payment_frequency
    ...
    def _float_leg_pv(self, trade: IRS, disc: YieldCurveInterpolator,
        ...
        cal = market.calendars.get(trade.calendar_name) if (market and trade.calendar_name) else None
        freq = trade.float_payment_frequency or trade.payment_frequency
        schedule = generate_schedule(trade.effective_date, trade.maturity_date,
                                     freq, calendar=cal,
                                     convention=trade.business_day_convention)
        s_d = trade.discount_spread
        ...
        for pay_date in schedule:
            ...
            fwd_rate = fwd.forward_rate(t_prev if t_prev > 1e-6 else 1e-6, t_pay) + s_f
    ...
    def _compute_pv01(self, trade: IRS, market: MarketSnapshot,
        ...
        bumped_npv = (float_pv - fixed_pv if trade.pay_receive == PayReceive.PAY
                      else fixed_pv - float_pv)
        return bumped_npv - base_npv


# ---------------------------------------------------------------------------
# Shared helper: generic floating leg
# ---------------------------------------------------------------------------
...
def _float_leg_pv(
    ...
    Parameters
    ----------
    notional_fn      : callable returning the outstanding notional for a payment date
    spread           : additive rate spread applied to every period's forward rate
    discount_spread  : additive spread on the discount-curve zero rate (decimal)
    forward_spread   : additional additive shift on the forward rate (decimal)
    """
    ...
    for pay_date in schedule:
        ...
        df = _sdf(disc, t_pay, discount_spread)
        notional = notional_fn(pay_date)
        pv += (fwd_rate + spread + forward_spread) * tau * df * notional
        prev = pay_date
    return pv
...
def _fixed_leg_pv(
    ...
    """Generic fixed-leg PV with support for amortizing notionals and discount spread."""
    from ..common.enums import BusinessDayConvention as _BDC
    _conv = convention if convention is not None else _BDC.MODIFIED_FOLLOWING
    schedule = generate_schedule(effective_date, maturity_date, frequency,
                                 calendar=calendar, convention=_conv)
    ...
    for pay_date in schedule:
        ...
        df = _sdf(disc, t, discount_spread)
        notional = notional_fn(pay_date)
...
class AmortizingIRSPricer(PricingEngine):
    ...
    def _compute_npv(self, trade: AmortizingIRS, market: MarketSnapshot) -> float:
        ...
        float_freq = trade.float_payment_frequency or trade.payment_frequency

        fixed_pv = _fixed_leg_pv(
            disc, trade.effective_date, trade.maturity_date,
            fixed_freq, trade.fixed_day_count, today,
            trade.fixed_rate, notional_fn,
            discount_spread=trade.discount_spread,
            calendar=cal, convention=conv,
        )
        float_pv = _float_leg_pv(
            disc, fwd, trade.effective_date, trade.maturity_date,
            float_freq, trade.float_day_count, today, notional_fn,
            discount_spread=trade.discount_spread,
            forward_spread=trade.forward_spread,
            calendar=cal, convention=conv,
        )
...
class FloatFloatSwapPricer(PricingEngine):
    ...
    def _compute_npv(self, trade: FloatFloatSwap, market: MarketSnapshot) -> float:
        ...
        leg1_pv = _float_leg_pv(
            ...
            discount_spread=s_d, calendar=cal, convention=conv,
        )
...
# ---------------------------------------------------------------------------
# Amortizing float-float (basis) swap pricer
# ---------------------------------------------------------------------------

class AmortizingFloatFloatSwapPricer(PricingEngine):
    """
    Price a floating-for-floating (basis) swap with a step-down or step-up notional.

    Identical to ``FloatFloatSwapPricer`` except each accrual period uses the
    notional returned by ``trade.notional_at(payment_date)``.
    """

    def price(self, trade: AmortizingFloatFloatSwap,  # type: ignore[override]
    ...
    def _compute_npv(self, trade: AmortizingFloatFloatSwap,
    ...
    def _price(self, trade: AmortizingFloatFloatSwap,
               market: MarketSnapshot) -> PricingResult:
        npv = self._compute_npv(trade, market)
        pv01 = self._compute_pv01(trade, market, npv)
        return PricingResult(
    ...
    def _compute_pv01(self, trade: AmortizingFloatFloatSwap,
                      market: MarketSnapshot, base_npv: float) -> float:
        bump = 0.0001
        orig = market.yield_curves[trade.discount_curve_id]
        bumped_curves = dict(market.yield_curves)
        bumped_curves[trade.discount_curve_id] = YieldCurve(
